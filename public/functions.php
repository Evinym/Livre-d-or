...
function get_event_list(){
    ...
    try{
        return $reponse = $connection->query("SELECT * FROM livreurs");
...
function add_event($name, $surname, $passwordd, $id=null){
    ...
    if($id){
        $sql = "UPDATE livreurs SET name = ?, surname = ?, passwordd = ? WHERE id = ?";
    } else {
        $sql = "INSERT INTO livreurs (name, surname, passwordd) VALUE(?, ?, ?)";
    }
