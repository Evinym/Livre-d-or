...
function add_books($titre, $date, $hour, $description, $image, $livre, $name, $id=null){
    include "connection.php";

    if($id){
        $sql = "UPDATE event SET titre = ?, date = ?, hour = ?, description = ?, image = ?, livre = ?, name = ? WHERE id = ?";
    } else {
        $sql = "INSERT INTO event (titre, date, hour, description, image, livre, name) VALUE(?, ?, ?, ?, ?, ?, ?)";
    }
